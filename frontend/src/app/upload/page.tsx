...
export default function UploadPage() {
  ...
  const SUPPORTED_EXTENSIONS = ['.pdf', '.png', '.jpg', '.jpeg', '.webp', '.gif', '.docx', '.doc', '.xlsx', '.xls', '.txt', '.csv'];
  ...
  const analyzeFile = async (fileWithStatus: FileWithStatus, index: number): Promise<AnalysisResult | null> => {
    ...
    try {
      console.log(`Analiz başlıyor (${index + 1}/${files.length}):`, file.name);
  ...
  return (
    <Box style={{ background: 'linear-gradient(180deg, rgba(139,92,246,0.05) 0%, rgba(255,255,255,0) 100%)', minHeight: '100vh' }}>
        ...
        <Stack gap="xl">
          ...
          <Box ta="center">
            ...
            <Group justify="center" mt="md" gap="xs">
              ...
              <Badge color="green" variant="light">PNG/JPG</Badge>
              <Badge color="orange" variant="light">Word</Badge>
              <Badge color="grape" variant="light">Excel</Badge>
              <Badge color="gray" variant="light">TXT</Badge>
            </Group>
          </Box>
          ...
          <Alert icon={<IconAlertCircle size={16} />} title="Claude AI Özellikleri" color="violet" variant="light">
            <List size="sm" spacing="xs">
              <List.Item>📄 <strong>PDF:</strong> Sayfa sayfa görsel analiz</List.Item>
              <List.Item>🖼️ <strong>Görseller:</strong> PNG, JPG, WEBP doğrudan analiz</List.Item>
              <List.Item>📝 <strong>Word:</strong> DOCX/DOC metin çıkarma ve analiz</List.Item>
              <List.Item>📊 <strong>Excel:</strong> XLSX/XLS tablo analizi</List.Item>
              <List.Item>📋 <strong>Metin:</strong> TXT/CSV dosya analizi</List.Item>
            </List>
          </Alert>
                ...
                <Stack gap="xs">
                  {files.map((fileItem, index) => (
                    <Paper key={index} p="sm" withBorder style={{ 
                      borderColor: fileItem.status === 'completed' ? 'var(--mantine-color-green-5)' : 
                                   fileItem.status === 'error' ? 'var(--mantine-color-red-5)' : 
                                   fileItem.status === 'analyzing' ? 'var(--mantine-color-blue-5)' : undefined
                    }}>
                      <Group justify="space-between">
                        <Group>
                          <ThemeIcon 
                            ...
                            variant="light"
                            color={
                              fileItem.status === 'completed' ? 'green' : 
                              fileItem.status === 'error' ? 'red' : 
                              fileItem.status === 'analyzing' ? 'blue' : 'gray'
                            }
                          >
                            {fileItem.status === 'completed' ? <IconCheck size={18} /> :
                             fileItem.status === 'error' ? <IconX size={18} /> :
                             fileItem.status === 'analyzing' ? <IconFileAnalytics size={18} /> :
                             <IconFile size={18} />}
                          </ThemeIcon>
                          <div>
                            <Text size="sm" fw={500}>{fileItem.file.name}</Text>
                            <Text size="xs" c="dimmed">
                              {formatFileSize(fileItem.file.size)} • {getFileExtension(fileItem.file.name).toUpperCase().slice(1)}
                              {fileItem.status === 'analyzing' && fileItem.progress && ` • ${fileItem.progress.message}`}
                        ...
                        <Group gap="xs">
                          ...
                          >
                            {fileItem.status === 'completed' ? '✓ Tamamlandı' : 
                             fileItem.status === 'error' ? '✗ Hata' : 
                             fileItem.status === 'analyzing' ? '⟳ Analiz...' : 'Bekliyor'}
                          </Badge>
                          {!uploading && (
                            <Button 
                              variant="subtle" 
          ...
          {combinedResult && (
            <Card withBorder p={0} style={{ overflow: 'hidden' }}>
              {/* Header */}
              <Box p="lg" style={{ background: 'linear-gradient(135deg, var(--mantine-color-green-6) 0%, var(--mantine-color-teal-6) 100%)' }}>
                <Group justify="space-between">
                  <Group>
                    ...
                    <div>
                      <Title order={2} c="white">✅ Analiz Tamamlandı</Title>
                ...
                <SimpleGrid cols={{ base: 2, sm: 3, md: 5 }} spacing="md">
                  ...
                  {combinedResult.tarih && (
                    <Paper p="md" withBorder radius="md">
                      ...
                      <Text size="sm" fw={500}>{combinedResult.tarih}</Text>
                    </Paper>
                  )}
                  ...
                  {combinedResult.sure && (
                    <Paper p="md" withBorder radius="md">
                      ...
                      <Text size="sm" fw={500}>{combinedResult.sure}</Text>
                    </Paper>
                  )}
                </SimpleGrid>
              </Box>
                ...
                <Tabs defaultValue="teknik" variant="outline">
                  ...
                  <Tabs.Panel value="notlar" pt="md">
                    {combinedResult.notlar?.length > 0 ? (
                      ...
                      <Text c="dimmed" ta="center" py="xl">Not bulunamadı</Text>
                    )}
                  </Tabs.Panel>
                  ...
                  <Tabs.Panel value="metin" pt="md">
                    {combinedResult.tam_metin ? (
                      ...
                      <Text c="dimmed" ta="center" py="xl">Metin bulunamadı</Text>
                    )}
                  </Tabs.Panel>
                </Tabs>
              </Box>
          ...
          {/* Claude Özellikleri */}
          <Card shadow="md" padding="xl" radius="lg" withBorder>
            <Title order={3} mb="md">🤖 Claude AI Özellikleri</Title>
            <List spacing="md" icon={
              ...
              <List.Item>
                ...
                <Text size="sm" c="dimmed">PDF'i sayfa sayfa görsele çevirerek analiz eder</Text>
              </List.Item>
              <List.Item>
                <Text fw={500}>Akıllı Metin Tanıma</Text>
                <Text size="sm" c="dimmed">El yazısı ve baskı metinleri %99 doğrulukla okur</Text>
              </List.Item>
              <List.Item>
                <Text fw={500}>Tablo ve Form Tanıma</Text>
                <Text size="sm" c="dimmed">Karmaşık tabloları ve formları yapısal olarak çıkarır</Text>
              </List.Item>
              <List.Item>
                <Text fw={500}>İçerik Analizi</Text>
                <Text size="sm" c="dimmed">İhale şartnamelerine özel bilgi çıkarımı yapar</Text>
              </List.Item>
            </List>
          </Card>
        </Stack>
