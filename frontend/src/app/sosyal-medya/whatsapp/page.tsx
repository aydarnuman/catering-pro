...
export default function WhatsAppPage() {
  ...
  const convertToBlob = useCallback(async (dataUrl: string, _filename: string) => {
    try {
      ...
      if (!matches) {
        console.debug('Not a base64 data URL, using directly');
        return dataUrl;
      }
      ...
      console.debug('Created blob URL', { blobUrl });
      return blobUrl;
    } catch (error) {
    ...
    const processPreview = async () => {
      ...
      try {
        ...
        if (previewFilename.match(/\.docx?$/i)) {
          console.debug('Processing DOCX with Mammoth.js...');
          ...
          if (matches) {
            ...
            console.debug('Mammoth conversion successful');
  ...
  const fetchChats = useCallback(async () => {
    try {
      ...
      console.error('Failed to fetch chats', { error: e });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        console.error('WhatsApp status check failed', { error: e });
  ...
  const markAsRead = useCallback(async (chatId: string) => {
    try {
      ...
      console.error('Failed to mark as read', { error: e });
    }
  }, []);
  ...
  const downloadMedia = async (messageId: string) => {
    ...
    try {
      ...
      console.error('Failed to download media', { error: e });
      notifications.show({
        title: 'Bağlantı Hatası',
  ...
  const saveMediaToServer = async (messageId: string) => {
    ...
    try {
      ...
      console.error('Failed to save media', { error: e });
      notifications.show({
        title: 'Hata',
