...
export function UrunDetayModal({ opened, onClose, urun, isMobile, isMounted, onFiyatTrendiSec }: UrunDetayModalProps) {
  ...
  const { data: sonIslemler = [], isLoading: sonIslemlerLoading } = useQuery<SonIslemRow[]>({
    ...
    queryFn: async (): Promise<SonIslemRow[]> => {
      ...
      const rows = (await faturaKalemleriAPI.getFiyatGecmisi(String(urunId), 10)) as FiyatGecmisiItem[];
      return rows.map((r, i) => ({
        id: i,
  ...
  const { data: tedarikciAnalizi = [], isLoading: tedarikciLoading } = useQuery<TedarikciAnalizRow[]>({
  ...
  const { data: miniTrendData = [] } = useQuery<PriceHistoryData[]>({
    ...
    queryFn: async (): Promise<PriceHistoryData[]> => {
      const urunId = urun?.urun_id;
      if (urunId == null) return [];
      const rows = (await faturaKalemleriAPI.getFiyatGecmisi(String(urunId), 100)) as FiyatGecmisiItem[];
  ...
  const zamanKarsilastirma = useMemo(() => {
    ...
    const sorted = [...miniTrendData].sort((a, b) => new Date(a.month).getTime() - new Date(b.month).getTime());
    ...
    const rafOrt =
      rafFiyatlar.length > 0
        ? rafFiyatlar
            ...
            .filter((f) => f > 0)
            .reduce((a, b) => a + b, 0) /
          (rafFiyatlar.filter((r) => Number(r.birim_fiyat || r.piyasa_fiyat_ort || 0) > 0).length || 1)
        : 0;
  ...
  const handleFiyatKaynakSec = useCallback(
    async (tipi: 'FATURA' | 'PIYASA' | 'MANUEL', fiyat?: number) => {
      ...
      try {
        ...
        notifications.show({
          title: 'Hata',
          message: backendMsg || (err instanceof Error ? err.message : 'Fiyat kaynağı değiştirilemedi'),
          color: 'red',
        });
      } finally {
  ...
  const exportToExcel = useCallback(() => {
    if (!urun) return;
    const csvData = [
      ['Ürün Adı', 'Toplam Tutar', 'Ort. Fiyat', 'Min', 'Max', 'Miktar', 'Fatura Sayısı', 'Kategori'],
      [
        urun.product_name,
                  ...
                  <Box ta="right">
                    ...
                    <Group gap={6} justify="flex-end">
                      ...
                      <Text
                        fw={700}
                        size="lg"
                        c={zamanKarsilastirma.fark > 0 ? 'red' : zamanKarsilastirma.fark < 0 ? 'green' : 'dimmed'}
                      >
                        {zamanKarsilastirma.fark > 0 ? '+' : ''}
                    ...
                    <Text size="xs" c="dimmed">
                      ₺{zamanKarsilastirma.gecenAy.toFixed(2)} → ₺{zamanKarsilastirma.buAy.toFixed(2)}
                    </Text>
                  </Box>
                )}
              </Group>
                ...
                <Group gap={6} grow>
                  ...
                  <Box
                    ...
                    style={{
                      ...
                      background:
                        fiyatBilgi.aktifTipi === 'PIYASA' ? 'rgba(253, 126, 20, 0.08)' : 'var(--mantine-color-dark-7)',
                  ...
                  <Box
                    ...
                    style={{
                      ...
                      background:
                        fiyatBilgi.aktifTipi === 'FATURA' ? 'rgba(34, 139, 230, 0.08)' : 'var(--mantine-color-dark-7)',
                  ...
                  >
                    ...
                    <Text size="sm" fw={700} c={fiyatBilgi.faturaFiyatDeger > 0 ? 'blue.4' : 'dimmed'}>
                      {fiyatBilgi.faturaFiyatDeger > 0 ? `₺${fiyatBilgi.faturaFiyatDeger.toFixed(2)}` : '—'}
                    </Text>
                  </Box>
                  ...
                  >
                    ...
                    <Text size="sm" fw={700} c={fiyatBilgi.manuelFiyatDeger > 0 ? 'violet.4' : 'dimmed'}>
                      {fiyatBilgi.manuelFiyatDeger > 0 ? `₺${fiyatBilgi.manuelFiyatDeger.toFixed(2)}` : '—'}
                    </Text>
                  </Box>
                </Group>
                ...
                <Stack gap="md" p="lg">
                  ...
                  {d.invoice_count > 0 && (
                    <SimpleGrid cols={{ base: 2, sm: 4 }} spacing="sm">
                      <StatCard label="Toplam Tutar" value={formatMoney(d.total_amount)} color="grape" />
                      <StatCard
                        label="Ortalama"
                        value={`₺${d.avg_unit_price?.toFixed(2) || '0'}`}
                        color="blue"
                        subtitle={
                          d.avg_unit_price ? calculateUnitPrice(d.avg_unit_price, d.product_name)?.display : undefined
                        }
                      />
                      <StatCard label="Minimum" value={`₺${d.min_unit_price?.toFixed(2) || '0'}`} color="green" />
                      <StatCard label="Maksimum" value={`₺${d.max_unit_price?.toFixed(2) || '0'}`} color="red" />
                    </SimpleGrid>
                  )}
                    ...
                    <Stack gap={1}>
                      {sonIslemler.map((item, idx) => (
                        <Box
                          ...
                          style={{
                            borderBottom:
                              idx < sonIslemler.length - 1 ? '1px solid var(--mantine-color-dark-5)' : 'none',
                          }}
                        >
                      ...
                      {tedarikciAnalizi
                        ...
                        .map((supplier, index) => (
                          <Box
                            ...
                            style={{
                              borderRadius: 'var(--mantine-radius-md)',
                              background: index === 0 ? 'var(--mantine-color-dark-6)' : 'transparent',
                              border:
                                index === 0
                ...
                <Stack gap="md" p="lg">
                  {varyantLoading ? (
                    ...
                    <>
                      ...
                      {varyantOzet && varyantOzet.varyant_sayisi > 0 && (
                        <SimpleGrid cols={{ base: 2, sm: 4 }} spacing="sm">
                          <StatCard label="Toplam Varyant" value={String(varyantOzet.varyant_sayisi)} color="violet" />
                          <StatCard label="Fiyatlı" value={String(varyantOzet.fiyatli_varyant_sayisi)} color="green" />
                          <StatCard
                            label="En Ucuz"
                          ...
                          <StatCard
                            label="Ortalama"
                            value={
                              varyantOzet.ortalama_fiyat ? `₺${Number(varyantOzet.ortalama_fiyat).toFixed(2)}` : '—'
                            }
                            color="blue"
                          />
                        </SimpleGrid>
