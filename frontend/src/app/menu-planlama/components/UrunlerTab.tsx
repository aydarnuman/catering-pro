...
import { type FiyatGecmisiItem, faturaKalemleriAPI, type PriceHistoryData } from '@/lib/api/services/fatura-kalemleri';
import { menuPlanlamaAPI, type PiyasaSyncDurum, type UrunKartiFiyat } from '@/lib/api/services/menu-planlama';
...
function getFiyatKaynagi(urun: UrunKartiFiyat): {
  ...
  if (urun.aktif_fiyat && Number(urun.aktif_fiyat) > 0) return { label: 'Varsayılan', color: 'grape', icon: IconCoin };
  ...
  if (urun.manuel_fiyat && Number(urun.manuel_fiyat) > 0) return { label: 'Manuel', color: 'blue', icon: IconCoin };
  // Varyant fallback: kendi fiyatı yok ama varyantlardan gelen fiyat var
  if (Number(urun.varyant_en_ucuz) > 0) return { label: 'Varyant', color: 'violet', icon: IconPackages };
...
export function UrunlerTab({ isActive, isMobile = false, isMounted = true }: UrunlerTabProps) {
  ...
  const {
    ...
    queryFn: async (): Promise<PriceHistoryData[]> => {
      ...
      const rows = (await faturaKalemleriAPI.getFiyatGecmisi(String(seciliUrunId), 500)) as FiyatGecmisiItem[];
      ...
      {!isLoading && !urunlerError && (
        <Box
          ...
          style={{
            borderRadius: 'var(--mantine-radius-md)',
            background: 'var(--mantine-color-dark-6)',
            border: syncDurum?.isRunning ? '1px solid var(--mantine-color-blue-8)' : '1px solid transparent',
          }}
        >
          <Group justify="space-between" wrap="wrap" gap="sm">
            <Group gap="sm">
              <ThemeIcon size="sm" variant="light" color={syncDurum?.isRunning ? 'blue' : 'gray'} radius="xl">
                {syncDurum?.isRunning ? <Loader size={12} color="blue" /> : <IconRefresh size={12} />}
              </ThemeIcon>
              <Box>
                <Text size="xs" fw={500}>
            ...
            <Group gap="xs">
              {syncDurum?.stats?.totalProductsUpdated != null && syncDurum.stats.totalProductsUpdated > 0 && (
              ...
              >
                <Button
                  ...
                  variant="light"
                  color="blue"
                  leftSection={syncDurum?.isRunning ? <Loader size={12} /> : <IconRefresh size={14} />}
