...
export const reportsAPI = {
  ...
  async generate(reportId: string, format: 'excel' | 'pdf' = 'pdf', context: any = {}): Promise<Blob> {
    const { data } = await api.post('/api/reports/generate', { reportId, format, context }, {
      responseType: 'blob',
    });
    return data;
  },
   ...
   */
  async preview(reportId: string, format: 'excel' | 'pdf' = 'pdf', context: any = {}): Promise<{ type: 'pdf' | 'table'; data: Blob | { headers: string[]; rows: any[] } }> {
    const { data, headers } = await api.post('/api/reports/preview', { reportId, format, context }, {
      responseType: 'blob',
    });
  ...
  async bulk(reports: ReportRequest[]): Promise<Blob> {
    const { data } = await api.post('/api/reports/bulk', { reports }, {
      responseType: 'blob',
    });
    return data;
  },
   ...
   */
  async sendMail(reports: ReportRequest[], email: string, subject?: string): Promise<{ success: boolean; message: string }> {
