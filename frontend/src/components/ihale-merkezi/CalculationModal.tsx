...
export function CalculationModal({ opened, onClose, tender, onRefresh }: CalculationModalProps) {
  ...
  // KİK Sınır Değer için teklif listesi
  ...
  const toplamOgun = hesaplamaVerileri.toplam_ogun_sayisi || 
    (analysisSummary?.ogun_bilgileri?.reduce((sum: number, o: any) => sum + (Number(o.miktar) || 0), 0) || 0);
  ...
  const farkYuzde = aktifSinirDeger > 0 && bizimTeklif > 0 ? ((bizimTeklif - aktifSinirDeger) / aktifSinirDeger) * 100 : 0;
  ...
  const hesaplaKikSinirDeger = () => {
    ...
    // K katsayısı
    const kValues: Record<number, number> = { 3: 1.35, 4: 1.25, 5: 1.18, 6: 1.13, 7: 1.09, 8: 1.06 };
    ...
    notifications.show({
      ...
      message: `${n} teklif üzerinden KİK formülü uygulandı`,
      color: 'green',
    });
  };
  ...
  useEffect(() => {
    if (opened) {
      ...
      if (saved.teklifListesi) setTeklifListesi(saved.teklifListesi);
        ...
        <Stack gap="md">
          ...
          <Paper p="md" bg="dark.7" radius="md">
            <Group justify="space-between" mb="xs">
              <Text size="sm" c="dimmed">Sınır Değer (×0.85)</Text>
            ...
            {yaklasikMaliyet === 0 ? (
              <>
                <RingProgress
                  ...
                  label={<Text ta="center" size="sm" c="dimmed">?</Text>}
                />
                <Text c="dimmed" ta="center">Yaklaşık Maliyet girin</Text>
              </>
            ) : bizimTeklif === 0 ? (
              <>
                <RingProgress
                  ...
                  label={<Text ta="center" fw={700}>50%</Text>}
                />
                <Text c="dimmed" ta="center">Bizim Teklif girin</Text>
              </>
            ) : (
              <>
                <ThemeIcon
    ...
    <Stack gap="md">
      <Paper p="md" bg="rgba(201, 162, 39, 0.1)" radius="md" style={{ border: '1px solid var(--mantine-color-yellow-6)' }}>
        <Group gap="xs" mb="xs">
          ...
          <Text size="sm" fw={600} c="yellow">KİK Sınır Değer Formülü</Text>
        </Group>
        <Text size="xs" c="dimmed">
          Sınır Değer = Ortalama - (K × Standart Sapma). K katsayısı teklif sayısına göre belirlenir.
          En az 3 geçerli teklif gereklidir.
        </Text>
      </Paper>
              ...
              {teklifListesi.map((teklif, index) => (
                <Group key={`teklif-${teklif.firma}-${index}`} gap="xs">
                  <TextInput
                    placeholder="Firma"
                    value={teklif.firma}
                    onChange={(e) => setTeklifListesi(prev => 
                  ...
                  <NumberInput
                    placeholder="Tutar"
                    value={teklif.tutar || ''}
                    onChange={(val) => setTeklifListesi(prev =>
        ...
        <Paper p="lg" bg="dark.7" radius="md">
          <Stack align="center" justify="center" h="100%" gap="md">
            ...
            <Group justify="space-between" w="100%">
              <Text size="xs" c="dimmed">Basit Hesap (×0.85)</Text>
              <Text size="xs" fw={500}>{basitSinirDeger.toLocaleString('tr-TR')} ₺</Text>
            </Group>
            <Group justify="space-between" w="100%">
              <Text size="xs" c="dimmed">Teklif Sayısı</Text>
              <Text size="xs" fw={500}>{teklifListesi.filter(t => t.tutar > 0).length}</Text>
            </Group>
          </Stack>
        </Paper>
      </SimpleGrid>
    ...
    <Stack gap="md">
      <Paper p="md" bg="rgba(255, 107, 107, 0.1)" radius="md" style={{ border: '1px solid var(--mantine-color-red-6)' }}>
        ...
        <Text size="xs" c="dimmed">
          ...
          Toplam maliyetiniz teklifinizi karşılamalıdır.
        </Text>
      </Paper>
      ...
      <SimpleGrid cols={2} spacing="md">
        ...
        <Paper p="lg" bg="dark.7" radius="md">
          <Stack gap="md">
            <div>
              <Text size="xs" c="dimmed" mb={4}>Toplam Maliyet</Text>
              <Text size="xl" fw={700}>{toplamMaliyet.toLocaleString('tr-TR')} ₺</Text>
            </div>

            <div>
              <Text size="xs" c="dimmed" mb={4}>Bizim Teklif</Text>
              <Text size="lg" fw={600} c="blue">{bizimTeklif.toLocaleString('tr-TR')} ₺</Text>
            </div>
            ...
            <div>
              ...
              <Group gap="xs">
                ...
                {karMarji < 0 && (
                  <Badge color="red" size="sm">ZARAR</Badge>
                )}
              </Group>
            </div>
      ...
      <SimpleGrid cols={3} spacing="md">
        <Card padding="lg" radius="md" withBorder>
          <Stack align="center" gap="md">
            ...
            <Text size="xs" c="dimmed">Teklif ile birlikte</Text>
          </Stack>
        </Card>
        ...
        <Card padding="lg" radius="md" withBorder>
          <Stack align="center" gap="md">
            ...
            <Text size="xs" c="dimmed">Sözleşme imzasında</Text>
          </Stack>
        </Card>
        ...
        <Card padding="lg" radius="md" withBorder>
          <Stack align="center" gap="md">
            ...
            <Text size="xs" c="dimmed">(%9 toplam)</Text>
          </Stack>
        </Card>
      </SimpleGrid>
          ...
          <Table>
            ...
            <Table.Tbody>
              <Table.Tr>
                ...
                <Table.Td ta="right" fw={600}>{geciciTeminat.toLocaleString('tr-TR', { maximumFractionDigits: 0 })} ₺</Table.Td>
              </Table.Tr>
              <Table.Tr>
                <Table.Td>Kesin Teminat</Table.Td>
                <Table.Td ta="right">%6</Table.Td>
                <Table.Td ta="right" fw={600}>{kesinTeminat.toLocaleString('tr-TR', { maximumFractionDigits: 0 })} ₺</Table.Td>
              </Table.Tr>
              <Table.Tr>
                <Table.Td>Damga Vergisi</Table.Td>
                <Table.Td ta="right">‰9.48</Table.Td>
                <Table.Td ta="right" fw={600}>{(bizimTeklif * 0.00948).toLocaleString('tr-TR', { maximumFractionDigits: 0 })} ₺</Table.Td>
              </Table.Tr>
              <Table.Tr>
                <Table.Td>KİK Payı</Table.Td>
                <Table.Td ta="right">‰0.5</Table.Td>
                <Table.Td ta="right" fw={600}>{(bizimTeklif * 0.0005).toLocaleString('tr-TR', { maximumFractionDigits: 0 })} ₺</Table.Td>
              </Table.Tr>
              <Table.Tr style={{ background: 'var(--mantine-color-dark-6)' }}>
                ...
                <Table.Td ta="right" fw={700} c="orange">
                  {(geciciTeminat + kesinTeminat + bizimTeklif * 0.00948 + bizimTeklif * 0.0005).toLocaleString('tr-TR', { maximumFractionDigits: 0 })} ₺
                </Table.Td>
              </Table.Tr>
            </Table.Tbody>
          </Table>
      ...
      title={
        <Group gap="sm">
          ...
          <div>
            ...
            <Text size="xs" c="dimmed">Profesyonel teklif analizi ve risk değerlendirmesi</Text>
          </div>
        </Group>
      }
      size="900px"
    ...
    >
      <Stack gap="md">
        ...
        {(isSuresi || toplamOgun > 0) && (
          <Paper p="sm" bg="rgba(20, 184, 166, 0.08)" radius="md" style={{ border: '1px solid var(--mantine-color-teal-6)' }}>
            <Group gap="xs" mb="xs">
              <IconSparkles size={14} color="var(--mantine-color-teal-6)" />
              <Text size="xs" fw={600} c="teal">Döküman Analizinden Tespit Edildi</Text>
            </Group>
            <SimpleGrid cols={4} spacing="md">
              {isSuresi && (
                <Box>
                  <Text size="xs" c="dimmed">Süre</Text>
                  <Text size="sm" fw={600}>{isSuresi}</Text>
                </Box>
              )}
              {toplamOgun > 0 && (
                <Box>
                  <Text size="xs" c="dimmed">Toplam Öğün</Text>
                  <Text size="sm" fw={600}>{toplamOgun.toLocaleString('tr-TR')}</Text>
                </Box>
              )}
              {gunlukOgun > 0 && (
                <Box>
                  <Text size="xs" c="dimmed">Günlük Öğün</Text>
                  <Text size="sm" fw={600}>~{gunlukOgun.toLocaleString('tr-TR')}</Text>
                </Box>
              )}
              {(teknikSartSayisi > 0 || birimFiyatSayisi > 0) && (
                <Box>
                  <Text size="xs" c="dimmed">Döküman</Text>
                  <Text size="sm" fw={600}>{teknikSartSayisi} şart, {birimFiyatSayisi} kalem</Text>
                </Box>
              )}
            </SimpleGrid>
          </Paper>
