...
import type { AgentPersona, AttachmentDetailState, AttachmentType, OrbitAttachment, ToolResult } from '../types';

// ─── API helper (Axios wrapper) ─────────────────────────
...
async function apiFetch<T>(path: string, opts?: { method?: string; body?: string }): Promise<T> {
  ...
  const res =
    method === 'get' || method === 'delete'
      ? await api[method as 'get' | 'delete'](`/api${path}`)
      : await api[method as 'post' | 'put'](`/api${path}`, data);
  return res.data;
}
...
function mapNoteToAttachment(note: Record<string, unknown>): OrbitAttachment {
  ...
  return {
    ...
    files: Array.isArray(note.attachments)
      ...
      : [],
    tags: Array.isArray(note.tags) ? (note.tags as { id: number; name: string; color: string }[]) : [],
    metadata: meta,
...
export function useAttachments({ tenderId, enabled, analysisSummary }: UseAttachmentsProps) {
    ...
    try {
      ...
      if (data.success) {
        ...
        const unseeded = seeds.filter(
          (seed) => !dbAttachments.some((db) => db.metadata.analysis_field === seed.metadata.analysis_field)
        );
      ...
      notifications.show({
        ...
        message: err instanceof Error ? err.message : 'Eklentiler yuklenirken bir hata olustu.',
        color: 'red',
        autoClose: 5000,
      });
    } finally {
  ...
  const addAttachment = useCallback(
    async (input: {
      ...
      try {
        ...
        notifications.show({
          title: 'Eklenti Hatasi',
          message: err instanceof Error ? err.message : 'Eklenti eklenirken bir hata olustu.',
          color: 'red',
          autoClose: 5000,
        });
      }
  ...
  const updateAttachment = useCallback(
    async (id: string, updates: { title?: string; content?: string; pinned?: boolean; color?: string }) => {
      try {
        const data = await apiFetch<{ success: boolean; note: Record<string, unknown> }>(`/notes/${id}`, {
          method: 'PUT',
          body: JSON.stringify(updates),
        });
        if (data.success && data.note) {
          setAttachments((prev) => prev.map((a) => (a.id === id ? mapNoteToAttachment(data.note) : a)));
        }
      } catch (err) {
        notifications.show({
          title: 'Guncelleme Hatasi',
          message: err instanceof Error ? err.message : 'Eklenti guncellenirken bir hata olustu.',
          color: 'red',
          autoClose: 5000,
        });
      }
  ...
  const deleteAttachment = useCallback(async (id: string) => {
    try {
      ...
      notifications.show({
        title: 'Silme Hatasi',
        message: err instanceof Error ? err.message : 'Eklenti silinirken bir hata olustu.',
        color: 'red',
        autoClose: 5000,
      });
    }
      ...
      switch (result.type) {
        ...
        case 'precedent':
          ...
          content = result.citations
            ? result.citations.map((c) => `**${c.reference}** (${c.relevance})\n${c.summary}`).join('\n\n---\n\n')
            : '';
          contentFormat = 'markdown';
          break;
        case 'redline':
  ...
  // ── Realtime sync — refetch when unified_notes changes ──
  const stableFetchRef = useRef(fetchAttachments);
  stableFetchRef.current = fetchAttachments;
  const realtimeRefetch = useCallback(() => stableFetchRef.current(), []);
  useRealtimeRefetch('unified_notes', realtimeRefetch);
