...
import type { AgentPersona, AttachmentType, OrbitAttachment } from '../types';
...
export function RingListPanel({
  ...
  if (!isExpanded) {
    return (
      ...
      >
        <Tooltip label="Ek Ogeler" position="left" withArrow>
          <ActionIcon
            variant="light"
            color="gray"
            size="xl"
      ...
      >
        ...
        >
          <Group justify="space-between" mb="xs">
            <Group gap="xs">
              <IconList size={18} color="rgba(255,255,255,0.5)" />
              <Text size="sm" fw={600} c="white">
                Ek Ogeler ({attachments.length})
              </Text>
            </Group>
            <ActionIcon
              variant="subtle"
...
function RingListItem({ attachment, onClick }: RingListItemProps) {
  const TypeIcon = TYPE_ICONS[attachment.type] || IconNote;
  const config = ATTACHMENT_TYPE_MAP[attachment.type];
  const agentColor = attachment.sourceAgent
    ? AGENT_COLORS[attachment.sourceAgent]
    : undefined;
  ...
  return (
    ...
    >
      {/* Virtual indicator - unsaved draft badge */}
      {attachment.virtual && (
        <Badge
          size="xs"
          variant="light"
