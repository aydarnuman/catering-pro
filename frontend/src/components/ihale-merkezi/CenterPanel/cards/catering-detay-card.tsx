...
export function CateringDetayKartlari({ analysisSummary }: { analysisSummary?: AnalysisData | null }) {
  ...
  const {
    kahvalti_kisi_sayisi, ogle_kisi_sayisi, aksam_kisi_sayisi, diyet_kisi_sayisi,
    hizmet_gun_sayisi, mutfak_tipi, servis_tipi, et_tipi, yemek_cesit_sayisi,
    yemek_pisirilecek_yer, iscilik_orani, dagitim_saatleri, dagitim_noktalari,
    ekipman_listesi, kalite_standartlari, gida_guvenligi_belgeleri,
    malzeme_listesi, ogun_dagilimi, birim_fiyat_cetveli, menu_tablosu,
  } = analysisSummary;

  const allItems: InfoItem[] = [
    { label: 'Kahvaltı', value: kahvalti_kisi_sayisi ?? '', section: 'kisi', fieldKey: 'kahvalti_kisi_sayisi', isLong: false },
    ...
    { label: 'Mutfak Tipi', value: mutfak_tipi ?? '', section: 'hizmet', fieldKey: 'mutfak_tipi', isLong: String(mutfak_tipi || '').length > 80 },
    { label: 'Servis Tipi', value: servis_tipi ?? '', section: 'hizmet', fieldKey: 'servis_tipi', isLong: String(servis_tipi || '').length > 80 },
    { label: 'Et Tipi', value: et_tipi ?? '', section: 'hizmet', fieldKey: 'et_tipi', isLong: String(et_tipi || '').length > 80 },
    { label: 'Pişirme Yeri', value: yemek_pisirilecek_yer ?? '', section: 'hizmet', fieldKey: 'yemek_pisirilecek_yer', isLong: String(yemek_pisirilecek_yer || '').length > 80 },
    { label: 'Çeşit Sayısı', value: yemek_cesit_sayisi ?? '', section: 'hizmet', fieldKey: 'yemek_cesit_sayisi', isLong: false },
    { label: 'Hizmet Günü', value: hizmet_gun_sayisi ?? '', section: 'hizmet', fieldKey: 'hizmet_gun_sayisi', isLong: false },
    { label: 'Dağıtım Saati', value: dagitim_saatleri ?? '', section: 'lojistik', fieldKey: 'dagitim_saatleri', isLong: String(dagitim_saatleri || '').length > 80 },
    { label: 'Dağıtım Noktaları', value: dagitim_noktalari ?? '', section: 'lojistik', fieldKey: 'dagitim_noktalari', isLong: String(dagitim_noktalari || '').length > 80 },
    { label: 'Ekipman', value: ekipman_listesi ?? '', section: 'lojistik', fieldKey: 'ekipman_listesi', isLong: String(ekipman_listesi || '').length > 80 },
    { label: 'Kalite Std.', value: kalite_standartlari ?? '', section: 'kalite', fieldKey: 'kalite_standartlari', isLong: String(kalite_standartlari || '').length > 80 },
    { label: 'Gıda Güv.', value: gida_guvenligi_belgeleri ?? '', section: 'kalite', fieldKey: 'gida_guvenligi_belgeleri', isLong: String(gida_guvenligi_belgeleri || '').length > 80 },
    { label: 'İşçilik Oranı', value: iscilik_orani ?? '', section: 'kalite', fieldKey: 'iscilik_orani', isLong: false },
    { label: 'Öğün Dağılımı', value: ogun_dagilimi ?? '', section: 'menu', fieldKey: 'ogun_dagilimi', isLong: String(ogun_dagilimi || '').length > 80 },
    { label: 'Malzeme Listesi', value: malzeme_listesi ?? '', section: 'menu', fieldKey: 'malzeme_listesi', isLong: String(malzeme_listesi || '').length > 80 },
    { label: 'Birim Fiyat', value: birim_fiyat_cetveli ?? '', section: 'menu', fieldKey: 'birim_fiyat_cetveli', isLong: String(birim_fiyat_cetveli || '').length > 80 },
    { label: 'Menü Tablosu', value: menu_tablosu ?? '', section: 'menu', fieldKey: 'menu_tablosu', isLong: String(menu_tablosu || '').length > 80 },
  ];
  ...
  return (
    <>
            ...
            <Group gap="xs" wrap="wrap">
              {kisiSection.items.map((item) => (
                <Badge key={`kisi-${item.label}`} size="sm" variant="light" color="blue">
                  {item.label}: {typeof item.value === 'number' ? Number(item.value).toLocaleString('tr-TR') : item.value} kişi
                </Badge>
              ))}
            </Group>
          )}
                ...
                {(expanded ? section.items : section.items.slice(0, 3)).map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {clickable && (
                        <IconChevronDown size={10} style={{ transform: 'rotate(-90deg)', flexShrink: 0, opacity: 0.5 }} />
                      )}
                    </Group>
                  );
                })}
      ...
      <AnalysisDetailModal
        ...
        icon={<IconToolsKitchen2 size={16} />}
        color="orange"
        data={filledItems.reduce((acc, item) => {
