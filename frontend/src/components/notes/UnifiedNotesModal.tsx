  ...
  const allNotesMarkdown = useMemo(() => {
    const md = notes
      .map((n) => {
        ...
        if (n.content) parts.push(stripHtmlContent(n.content));
        if (n.tags?.length)
          parts.push(`**Etiketler:** ${n.tags.map((t) => `\`${t.name}\``).join(' ')}`);
        return parts.join('\n\n');
      })
      .join('\n\n---\n\n');
        ...
        {calcOpen && (
          ...
          >
            <Group justify="space-between" mb="sm">
              ...
              <ActionIcon variant="subtle" size="sm" radius="md" onClick={() => setCalcOpen(false)}>
                <IconX size={14} />
              </ActionIcon>
            </Group>
            <ScrollArea mah={420}>
                            ...
                            isActive ? (
                              <ActionIcon
                                ...
                                onClick={(e) => {
                                  ...
                                  if (
                                    window.confirm(
                                      `"${f.name}" klasorunu silmek istediginize emin misiniz?`
                                    )
                                  ) {
                                    deleteFolder(f.id);
                                    setActiveFolderId(null);
