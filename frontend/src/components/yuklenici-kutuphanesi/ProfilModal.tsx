...
import { YapiskanNotlar } from './YapiskanNotlar';
...
export function ProfilModal({
  ...
  const fetchAiOzet = useCallback(async () => {
    ...
    try {
      ...
      if (json.success && json.data?.ozet) {
        ...
        notifications.show({
          title: 'Uyari',
          message: json.error || 'AI ozeti olusturulamadi',
          color: 'orange',
        });
      }
    } catch (err) {
  ...
  const handlePuanChange = async (newPuan: number) => {
    try {
      ...
      notifications.show({
        title: 'Kaydedildi',
        message: `Puan ${newPuan} olarak guncellendi`,
        color: 'green',
      });
    } catch (err) {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (!status.running) {
          ...
          fetchDetail();
          notifications.show({
            title: 'Istihbarat Tamamlandi',
            message: 'Ihale gecmisi guncellendi',
            color: 'green',
          });
        }
      } catch {
    ...
    return (
      <Box p="lg">
        <Group justify="space-between" mb="md">
          ...
          <ActionIcon variant="subtle" onClick={onClose} aria-label="Kapat">
            <IconX size={18} />
          </ActionIcon>
        </Group>
        <Stack gap="sm">
  ...
  return (
    <Box
      style={{
        ...
        height: '100vh',
        background: 'linear-gradient(180deg, #0f1015 0%, #16171c 100%)',
      }}
    >
              ...
              <Group gap={6} mt={2}>
                ...
                {yk.scraped_at && (
                  <Text size="xs" c="dimmed">
                    Son tarama: {formatZaman(yk.scraped_at)}
                  </Text>
                )}
              </Group>
            </div>
        ...
        <Group grow mt="sm">
          ...
          <Card p="xs" radius="md" className="yk-stat-card">
            ...
            <Text
              fw={700}
              size="lg"
              style={{ color: yk.devam_eden_is_sayisi > 0 ? 'var(--yk-gold-light)' : undefined }}
            >
      ...
      <Box style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <Group justify="space-between" mb="xs">
            <Group gap="sm">
              ...
              {yk.etiketler && yk.etiketler.length > 0 && (
                <Group gap={4}>
                  {yk.etiketler.map((e) => (
                    <Badge key={`etk-${e}`} size="xs" variant="light">
                      {e}
                    </Badge>
                  ))}
                </Group>
                ...
                >
                  <Stack align="center" gap={4}>
                    ...
                    <Text size="xs" c="dimmed">
                      {aiOzetLoading
                        ? 'Özet hazırlanıyor...'
                        : 'Firmanın ihale geçmişi, sektör ve performans verilerinden kısa bir özet oluşturur'}
                    </Text>
                  </Stack>
                </Card>
              )}

              {/* Sehir Dagilimi — gizlendi */}
                  ...
                  <Stack gap={4}>
                    {data.kazanilanIhaleler.slice(0, 5).map((ihale) => (
                      <Paper
                        ...
                        onMouseEnter={(e) => {
                          e.currentTarget.style.borderColor = 'var(--yk-border)';
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.borderColor = 'var(--yk-border-subtle)';
                        }}
                        onClick={() => {
                        ...
                        <Group justify="space-between" wrap="nowrap">
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0 }}>
                            {ihale.sozlesme_bedeli && (
                              <Text size="xs" fw={600} style={{ color: 'var(--yk-gold)' }}>
        ...
        <IstihbaratMerkezi
          ...
          isDark={isDark}
          yuklenici={yk}
          onYukleniciUpdate={(updated) => {
            if (data) {
              setData({ ...data, yuklenici: { ...yk, ...updated } });
            }
          }}
        />
      </Box>
