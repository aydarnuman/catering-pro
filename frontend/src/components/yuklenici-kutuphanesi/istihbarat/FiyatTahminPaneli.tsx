...
import { IconArrowDown, IconArrowRight, IconArrowUp, IconChartLine, IconInfoCircle, IconMapPin } from '@tabler/icons-react';
...
export function FiyatTahminPaneli({ yukleniciId }: Props) {
  ...
  if (hata) {
    return <Alert color="red" title="Hata">{hata}</Alert>;
  }
  ...
  const trendIkon = veri.trend === 'artiyor' ? <IconArrowUp size={16} /> : veri.trend === 'azaliyor' ? <IconArrowDown size={16} /> : <IconArrowRight size={16} />;
  const trendRenk = veri.trend === 'artiyor' ? 'red' : veri.trend === 'azaliyor' ? 'green' : 'blue';
  const trendMesaj = veri.trend === 'artiyor'
    ...
    <Stack gap="md">
      ...
      <Card withBorder p="sm" radius="sm">
        <Group gap="xs" mb="xs">
          ...
          <div>
            ...
            <Text size="xs" c="dimmed">{trendMesaj}</Text>
          </div>
        </Group>
        ...
        {veri.trend_detay && (
          <Group gap="md" mt="xs">
            <div>
              <Text size="xs" c="dimmed">Son 10 İhale Ort.</Text>
              <Text size="sm" fw={600}>%{veri.trend_detay.son_10_ort}</Text>
            </div>
            <div>
              <Text size="xs" c="dimmed">Önceki Ort.</Text>
              <Text size="sm" fw={600}>%{veri.trend_detay.onceki_ort}</Text>
            </div>
            <div>
              <Text size="xs" c="dimmed">Fark</Text>
              <Text size="sm" fw={600} c={trendRenk}>
                {veri.trend_detay.fark > 0 ? '+' : ''}{veri.trend_detay.fark} puan
              </Text>
            </div>
          </Group>
        )}
      ...
      {veri.sehir_bazli && veri.sehir_bazli.length > 0 && (
        <div>
          <Group gap="xs" mb="xs">
            <IconMapPin size={14} />
            <Text size="sm" fw={600}>Şehir Bazlı İndirim Ortalamaları</Text>
          </Group>
          <Stack gap={4}>
            {veri.sehir_bazli.map((s) => (
              <Group key={`sehir-${s.sehir}`} justify="space-between">
                <Group gap="xs">
                  <Text size="xs" w={100}>{s.sehir}</Text>
                ...
                <Group gap="xs" style={{ flex: 1, maxWidth: 200 }}>
                  <Progress
                    ...
                    style={{ flex: 1 }}
                  />
                  <Text size="xs" fw={600} w={50} ta="right">%{s.ort_indirim}</Text>
                </Group>
              </Group>
            ))}
          </Stack>
      ...
      <Alert icon={<IconChartLine size={16} />} color="blue" variant="light" title="İpucu">
        Bu veriler, firmanın gelecek ihalelerdeki muhtemel indirim oranını tahmin etmenize yardımcı olur.
        Yüksek indirim oranı agresif rekabeti, düşük indirim oranı tutucu stratejiyi gösterir.
      </Alert>
    </Stack>
  );
}
...
function StatKart({ baslik, deger, renk }: { baslik: string; deger: string; renk?: string }) {
  return (
    <Card withBorder p="xs" radius="sm">
      ...
      <Text size="sm" fw={700} c={renk}>{deger}</Text>
    </Card>
  );
}
