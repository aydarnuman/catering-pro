...
router.get('/sablonlar', async (req, res) => {
  try {
    ...
    const whereConditions = ['ms.aktif = true'];
    const params = [];
    let paramIndex = 1;
...
router.post('/sablonlar', async (req, res) => {
  try {
    ...
    // Kategori ID bul
    const kategoriResult = await query('SELECT id FROM maliyet_kategoriler WHERE kod = $1', [kategori_kod]);
    ...
    // Güncel veriyi getir
    const guncelSablon = await query('SELECT * FROM v_maliyet_sablon_ozet WHERE id = $1', [sablonId]);
  ...
  try {
    ...
    `,
      [
        ...
        ogun_ids,
        kisi_sayisi,
        parseInt(gunSayisi.rows[0].gun_sayisi, 10),
        etiketler,
      ]
    );
...
async function hesaplaSablonMaliyet(sablonId) {
  try {
    // Şablon bilgisini al
    const sablon = await query('SELECT kisi_sayisi FROM maliyet_menu_sablonlari WHERE id = $1', [sablonId]);
    ...
    for (const yemek of yemekler.rows) {
      ...
      if (yemek.recete_id) {
        ...
        // Besin değerleri
        const besinResult = await query('SELECT kalori, protein FROM receteler WHERE id = $1', [yemek.recete_id]);
        kalori = parseInt(besinResult.rows[0]?.kalori, 10) || 0;
...
router.post('/sablonlar/:id/hesapla', async (req, res) => {
  try {
    ...
    // Güncel veriyi getir
    const sablon = await query('SELECT * FROM v_maliyet_sablon_ozet WHERE id = $1', [id]);
...
router.get('/menu-ogunler', async (req, res) => {
  try {
    ...
    const whereConditions = ['mp.proje_id = $1'];
    const params = [proje_id];
    let paramIndex = 2;
  ...
  try {
    ...
    result.rows.forEach((row) => {
      ...
      kategoriler[altKat].yemekler.push({
        ...
        kalori: parseInt(row.kalori, 10) || 0,
...
router.get('/receteler/:id/maliyet', async (req, res) => {
  try {
    const { id } = req.params;
    const { fiyat_tipi: _fiyat_tipi = 'piyasa' } = req.query; // piyasa | sistem | manuel
