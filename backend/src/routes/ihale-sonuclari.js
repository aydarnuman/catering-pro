 ...
 *     tags: [İhale Sonuçları]
 */
router.get('/aktif-surecler', async (req, res) => {
    try {
...
router.post('/:id/rakip-teklif', async (req, res) => {
    try {
        ...
        const current = await query(
            'SELECT diger_teklifler FROM ihale_sonuclari WHERE id = $1',
            [id]
        );
        ...
        const result = await query(
            'UPDATE ihale_sonuclari SET diger_teklifler = $1 WHERE id = $2 RETURNING *',
            [JSON.stringify(teklifler), id]
        );
...
router.delete('/:id/rakip-teklif/:sira', async (req, res) => {
    try {
        ...
        teklifler = teklifler.filter(t => t.sira !== parseInt(sira));
...
router.patch('/:id/durum', async (req, res) => {
    try {
        ...
        const validDurumlar = [
            ...
            'kazandik', 'elendik', 'itiraz_edildi', 'kik_basvurusu', 'sonuclandi'
        ];
...
router.post('/:id/hesaplama-kaydet', async (req, res) => {
    try {
        ...
        const current = await query(
            'SELECT hesaplamalar FROM ihale_sonuclari WHERE id = $1',
            [id]
        );
        ...
        const result = await query(
            'UPDATE ihale_sonuclari SET hesaplamalar = $1 WHERE id = $2 RETURNING *',
            [JSON.stringify(hesaplamalar), id]
        );
...
router.post('/:id/ai-sohbet', async (req, res) => {
    try {
        ...
        const current = await query(
            'SELECT ai_sohbet_gecmisi FROM ihale_sonuclari WHERE id = $1',
            [id]
        );
        ...
        const result = await query(
            'UPDATE ihale_sonuclari SET ai_sohbet_gecmisi = $1 WHERE id = $2 RETURNING *',
            [JSON.stringify(sonSohbet), id]
        );
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const result = await query(
            'DELETE FROM ihale_sonuclari WHERE id = $1 RETURNING id, ihale_basligi',
            [id]
        );
...
router.post('/tracking-aktar/:tenderId', async (req, res) => {
    try {
        ...
        const existing = await query(
            'SELECT id FROM ihale_sonuclari WHERE tender_id = $1',
            [tenderId]
        );
