...
async function findOrCreateRecete(yemekAdi) {
  ...
  const existing = await query(
    ...
    [`%${yemekAdi.split('/')[0].split('+')[0].trim()}%`, PROJE_ID]
  );
...
async function getOrCreateMenuPlan() {
  const existing = await query(
    `SELECT id FROM menu_planlari WHERE proje_id = $1 AND baslangic_tarihi = '2026-01-01'`,
    [PROJE_ID]
  );
...
async function importAksamMenu() {
  console.log('🌙 AKŞAM YEMEĞİ MENÜSÜ AKTARILIYOR...\n');
  ...
  console.log(`\n\n✅ Akşam: ${eklenenGun} gün, ${eklenenYemek} yemek eklendi`);
}

async function importKahvaltiMenu() {
  console.log('\n☀️ KAHVALTI MENÜSÜ AKTARILIYOR...\n');
  ...
  console.log(`\n\n✅ Kahvaltı: ${eklenenGun} gün, ${eklenenYemek} yemek eklendi`);
}

async function main() {
  console.log('═══════════════════════════════════════════');
  console.log('   EXCEL → MENÜ PLANI DİREKT AKTARIM');
  console.log('═══════════════════════════════════════════\n');
  ...
  console.log('\n═══════════════════════════════════════════');
  console.log(`📊 TOPLAM: ${summary.rows[0].gun_sayisi} gün, ${summary.rows[0].yemek_sayisi} yemek`);
  console.log('═══════════════════════════════════════════');
