...
class SettingsVersionService {
  ...
  static async saveVersion(settingKey, settingValue, changedBy, changeNote = null) {
    try {
      const result = await query(
        'SELECT save_setting_version($1, $2, $3, $4) as version',
        [settingKey, JSON.stringify(settingValue), changedBy, changeNote]
      );
  ...
  static async saveVersions(settings, changedBy, changeNote = null) {
    ...
    for (const [key, value] of Object.entries(settings)) {
      try {
        const version = await this.saveVersion(key, value, changedBy, changeNote);
  ...
  static async getHistory(settingKey, limit = 50) {
    try {
      const result = await query(
        'SELECT * FROM get_setting_history($1, $2)',
        [settingKey, limit]
      );
      return result.rows;
    } catch (error) {
  ...
  static async getAllHistory(limit = 100) {
    try {
      const result = await query(
        'SELECT * FROM get_all_settings_history($1)',
        [limit]
      );
      return result.rows;
    } catch (error) {
  ...
  static async getVersion(settingKey, version) {
    try {
      const result = await query(
        'SELECT * FROM get_setting_version($1, $2)',
        [settingKey, version]
      );
  ...
  static async restoreVersion(settingKey, version, restoredBy) {
    try {
      // Versiyonu getir
      const versionData = await this.getVersion(settingKey, version);
