...
class TenderContentService {
  ...
  async createContentDocuments(tenderId) {
    console.log(`📄 İhale ${tenderId} içerikleri documents tablosuna kaydediliyor...`);
    ...
    try {
      ...
      if (tender.announcement_content && tender.announcement_content.trim()) {
        if (existingTypes.includes('announcement')) {
          ...
          try {
            ...
            console.log(`✅ İhale İlanı döküman oluşturuldu: ${docResult.rows[0].id}`);
          } catch (error) {
      ...
      if (tender.goods_services_content && 
        ...
        if (existingTypes.includes('goods_services')) {
          ...
          try {
            ...
            console.log(`✅ Mal/Hizmet Listesi döküman oluşturuldu: ${docResult.rows[0].id}`);
          } catch (error) {
      ...
      console.error('❌ İçerik dökümanları oluşturma hatası:', error);
      throw error;
    }
  }
