...
CREATE TABLE IF NOT EXISTS daily_audit_findings (
    ...
    sorun_tipi VARCHAR(100) NOT NULL, -- eksik_fiyat, anormal_maliyet, tekrar_yemek vb.
    onem_seviyesi audit_severity DEFAULT 'orta',
    
    -- İlgili kayıt
    ilgili_tablo VARCHAR(100), -- receteler, urun_kartlari, menu_planlari vb.
    ilgili_id INTEGER,
    ilgili_kod VARCHAR(100), -- Ürün kodu, reçete kodu vb.
    ilgili_ad VARCHAR(255), -- Ürün adı, reçete adı vb.
    
    -- Sorun detayı
    aciklama TEXT NOT NULL,
    detay_json JSONB DEFAULT '{}'::jsonb, -- Ek detaylar (mevcut değer, beklenen değer vb.)
    ...
    -- Önerilen düzeltme
    onerilen_duzeltme_json JSONB, -- {action: "update_price", params: {...}}
    ...
    -- İşlem bilgileri
    isleme_alan_kullanici_id INTEGER REFERENCES users(id),
...
CREATE TABLE IF NOT EXISTS daily_audit_fixes (
    ...
    onceki_deger_json JSONB, -- Önceki değer(ler)
    yeni_deger_json JSONB,   -- Yeni değer(ler)
...
COMMENT ON TABLE daily_audit_runs IS 'Günlük AI denetim çalıştırma kayıtları';
COMMENT ON TABLE daily_audit_findings IS 'Denetimde tespit edilen sorunlar ve AI önerileri';
COMMENT ON TABLE daily_audit_fixes IS 'Uygulanan düzeltmelerin geçmişi';
COMMENT ON TABLE daily_audit_config IS 'Denetim sistemi yapılandırma parametreleri';
